...
{
public:
	HuaweiNetwork(const string& username,
		const string& passwd,
...
protected:
	 ...
	 */
	virtual void SetMd5Buf(PPASSWORDFRM pBuf, const u_char ID, const u_char *chap) = 0;
...
private:
	HuaweiNetwork(const HuaweiNetwork&);
	HuaweiNetwork& operator=(const HuaweiNetwork&);
};
