							 ...
							 :m_fp(NULL)
{
	...
	strcpy_s(m_Data.username, _countof(m_Data.username), username.c_str());
	strcpy_s(m_Data.password, _countof(m_Data.password), passwd.c_str());
	strcpy_s(m_Data.nic, _countof(m_Data.nic), interfacename.c_str());
...
bool  HuaweiNetwork::OpenAdapter()
{
	...
	sprintf_s(pcap_filter, 100, "ether dst %x:%x:%x:%x:%x:%x and ether proto 0x888e",
...
bool HuaweiNetwork::SendVersion(const u_char Id)
{
	...
	*/
	...
	// 
	u_char buf[100] = {0};
	VERSIONFRM* pbuf = (VERSIONFRM*) buf;
	InitBuf(buf);
...
{
	u_char buf[100] = { 0 };
	USERNAMEFRM* pbuf = (USERNAMEFRM*) buf;
...
void HuaweiNetwork::GenerateVersion(u_char *buf)
{
	...
	sprintf_s(strMagic, 9, "%02x%02x%02x%02x", strTemp[0], strTemp[1], strTemp[2], strTemp[3]);
	//printf("%s\n",strMagic);
